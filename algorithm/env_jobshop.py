...
class JobshopEnv:
    ...
    def reset(self):
        ...
        self.solution_matrix = np.zeros([self.job_num, self.machine_num], dtype=float)
    ...
    def _get_state(self):
        if self.state_type == 0:  # 状态变量特征
            state = []
            state = np.append(state, (self.next_time_on_machine - self.current_time) / self.max_op_len)
            state = np.append(state, self.job_on_machine / self.job_num)
