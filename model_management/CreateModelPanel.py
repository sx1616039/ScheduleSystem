...
class CreateModelPanel(wx.Panel):
    ...
    def __init__(self, parent, order_path=None):
        ...
        self.text_file_name.AppendText('新模型1')
        label_file_path = wx.StaticText(file_panel, wx.ID_ANY, u"路径：", wx.DefaultPosition, wx.DefaultSize, 0)
        self.text_file_path = wx.TextCtrl(file_panel, wx.ID_ANY, pos=(0, 30), size=(200, 25), style=wx.TE_LEFT)
        self.text_file_path.AppendText('C:\\Users\\wxq\\Desktop\\ScheduleSystem-main\\models')
        ''' 选择路径按钮 '''
        ...
        ''' 生成订单按钮 '''
        btn_save = wx.Button(file_panel, wx.ID_ANY, u"保存模型", wx.DefaultPosition, wx.Size(100, 26), 0)
        ...
        self.combobox_representation = wx.ComboBox(parameter_panel, -1, size=wx.Size(80, -1), choices=self.representation_method)
        self.combobox_representation.SetSelection(0)
        # 动作空间
        ...
        self.combobox_action_space.SetSelection(0)
        # 回报函数
        ...
        self.combobox_reward.SetSelection(0)

        # 调度策略网络
        self.policy_networks = ['MLP', 'SPP']
        label_policy_networks = wx.StaticText(parameter_panel, wx.ID_ANY, u"调度策略网络:", wx.DefaultPosition, wx.DefaultSize, 0)
        self.combobox_policy_networks = wx.ComboBox(parameter_panel, -1, size=wx.Size(80, -1), choices=self.policy_networks)
        self.combobox_policy_networks.SetSelection(0)
        ...
        self.combobox_optimizer.SetSelection(0)
        ...
        self.text_epsilon = wx.TextCtrl(self.RL_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                      style=wx.TE_LEFT)
        self.text_epsilon.AppendText('0.2')
        ...
        self.text_updates = wx.TextCtrl(self.RL_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                        style=wx.TE_LEFT)
        self.text_updates.AppendText('10')
    ...
    def on_button_save(self, event):
        save_path = self.text_file_path.GetLineText(0)
        model_name = self.text_file_name.GetLineText(0)
        dir_path = os.path.join(save_path, model_name)
        print("创建目录" + dir_path)
        os.makedirs(dir_path)
        file_name = model_name+'_description.xml'
        file_path = os.path.join(dir_path, file_name)

        root = et.Element('DRL_model')
        ...
        if policy == 0:
            ...
            spp_critic = Element("Critic_SPP",
                                 {'kernel_size': str(kernel_size), 'input_channel': str(input_channel),
                                  'output_channel': str(output_channel), 'padding': str(padding),
                                  'spp_level': str(spp_level), 'pooling_type': str(pooling_type),
                                  'full_connect_layers': str(full_connect_layers),
                                  'full_connect_dim': str(full_connect_dim), 'lr': str(lr),
                                  'activation': str(activation)})
            root.append(spp_actor)
            root.append(spp_critic)
        ...
        ppo = Element("ppo", {'optimizer': str(optimizer), 'gamma': str(gamma), 'epsilon': str(epsilon), 'updates': str(updates)})
        root.append(base)
        root.append(ppo)
    ...
    def show_MLP(self):
        ...
        self.text_hidden_layers = wx.TextCtrl(self.networks_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                              style=wx.TE_LEFT)
        self.text_hidden_layers.AppendText('1')
        ...
        self.text_hidden_layer_dim = wx.TextCtrl(self.networks_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                                 style=wx.TE_LEFT)
        self.text_hidden_layer_dim.AppendText('256')
        ...
        self.combobox_actor_af.SetSelection(1)
        ...
        self.text_critic_hidden_layers = wx.TextCtrl(self.networks_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                                     style=wx.TE_LEFT)
        self.text_critic_hidden_layers.AppendText('1')
        ...
        self.text_critic_hidden_layer_dim = wx.TextCtrl(self.networks_panel, wx.ID_ANY, pos=(0, 30), size=(50, 25),
                                                        style=wx.TE_LEFT)
        self.text_critic_hidden_layer_dim.AppendText('256')
