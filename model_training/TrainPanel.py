...
class TrainPanel(wx.Panel):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_model = wx.Button(calib_panel, wx.ID_ANY, u"参数设置",
        ...
        self.btn_opt = wx.Button(calib_panel, wx.ID_ANY, u"模型训练",
                                 wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.btn_view = wx.Button(calib_panel, wx.ID_ANY, u"训练数据生成",
                                 wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.btn_explore = wx.Button(calib_panel, wx.ID_ANY, u"结果分析",
                                  wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.trainNotebook = TrainNotebook.TrainNotebook(tree_panel)
    ...
    def on_button_model(self, event):
        if self.is_selected():
            self.trainNotebook.show_modeling_page()

    def on_button_opt(self, event):
        if self.is_selected():
            self.trainNotebook.show_opt_page()

    def on_button_view(self, event):
        if self.is_selected():
            self.trainNotebook.show_view_page()

    def on_button_explore(self, event):
        if self.is_selected():
            self.trainNotebook.show_explore_page()
